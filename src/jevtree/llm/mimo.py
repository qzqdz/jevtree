...
Reads gitignored repo `.env` (JEVTREE_LLM_*). Manual parse — no python-dotenv.
...
def _env_llm(name: str, default: str = "") -> str:
    """Prefer JEVTREE_LLM_*; fall back to legacy META_JEV_LLM_* for one release."""
    new_key = f"JEVTREE_LLM_{name}"
    old_key = f"META_JEV_LLM_{name}"
    val = os.environ.get(new_key) or os.environ.get(old_key) or default
    return val


def _cfg() -> tuple[str, str, str]:
    load_dotenv_env()
    base = _env_llm("BASE_URL", "").rstrip("/")
    model = _env_llm("MODEL", "")
    key = _env_llm("API_KEY", "")
    if not base or not model:
        raise RuntimeError(
            "JEVTREE_LLM_BASE_URL and JEVTREE_LLM_MODEL must be set in .env "
            "(legacy META_JEV_LLM_* still accepted)"
        )
